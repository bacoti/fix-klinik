        ...
        <form method="POST" action="{{ route('medicines.update', $medicine) }}" class="space-y-6">
            ...
            <div>
                ...
                <select name="type" id="type" required
                    ...
                    <option value="capsule" {{ old('type', $medicine->type) == 'capsule' ? 'selected' : '' }}>Kapsul</option>
                    <option value="syrup" {{ old('type', $medicine->type) == 'syrup' ? 'selected' : '' }}>Sirup</option>
                    <option value="injection" {{ old('type', $medicine->type) == 'injection' ? 'selected' : '' }}>Injeksi</option>
                    <option value="cream" {{ old('type', $medicine->type) == 'cream' ? 'selected' : '' }}>Krim</option>
                    <option value="ointment" {{ old('type', $medicine->type) == 'ointment' ? 'selected' : '' }}>Salep</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $medicine->description) }}</textarea>
